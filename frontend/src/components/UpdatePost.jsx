...
export default function UpdatePost() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/api/v1/post/getposts?postId=${postId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `http://localhost:3000/api/v1/post/updatepost/${postId}/${currentUser._id}`,formData,{withCredentials:true});
