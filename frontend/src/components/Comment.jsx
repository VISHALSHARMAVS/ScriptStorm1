...
export default function Comment({ comment , onLike,onEdit,onDelete}) {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        const res = await axios.get(`api/v1/user/${comment.userId}`);
  ...
  const handleSave = async () => {
    try {
      const res  = await axios.put(`api/v1/comment/editComment/${comment._id}`,editedContent, {withCredentials:true});
      if (res.status==200) {
        setIsEditing(false);
