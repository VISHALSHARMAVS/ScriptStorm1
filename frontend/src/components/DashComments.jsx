...
export default function DashComments() {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const res = await axios.get('https://scriptstorm1.onrender.com/api/v1/comment/getcomments',{withCredentials:true});
        if (res.status === 200) {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await axios.get(`https://scriptstorm1.onrender.com/api/v1/comment/getcomments?startIndex=${startIndex}`,{withCredentials:true});
      if (res.status === 200) {
  ...
  const handleDeleteComment = async () => {
    setShowModal(false);
    try {
      const res = await axios.delete(`https://scriptstorm1.onrender.com/api/v1/comment/deleteComment/${commentIdToDelete}`,{withCredentials:true});
      if (res.status === 200) {
