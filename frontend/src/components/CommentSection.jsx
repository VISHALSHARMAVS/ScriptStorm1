...
export default function CommentSection({ postId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://scriptstorm-47.onrender.com/api/v1/comment/create', {
        content: comment,
        postId,
  ...
  useEffect(() => {
    const getComments = async () => {
      try {
        const res = await axios.get(`https://scriptstorm-47.onrender.com/api/v1/comment/getPostComments/${postId}`);
        if (res.status === 200) {
          const data =  res.data;
          setComments(data);
        }
  ...
  const handleLike = async (commentId) => {
    try {
      ...
      const res = await axios.put(`https://scriptstorm-47.onrender.com/api/v1/comment/likeComment/${commentId}`,{}, {withCredentials:true});
      if (res.status==200) {
  ...
  const handleDelete = async (commentId) => {
    ...
    try {
      ...
      const res = await axios.delete(`https://scriptstorm-47.onrender.com/api/v1/comment/deleteComment/${commentId}`, {withCredentials:true});
      if (res.status==200) {
        // const data = await res.data;
