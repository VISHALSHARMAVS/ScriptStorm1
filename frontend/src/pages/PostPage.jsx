...
export default function PostPage() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`http://localhost:3000/api/v1/post/getposts?slug=${postSlug}`);
  ...
  useEffect(() => {
    try {
      const fetchRecentPosts = async () => {
        const res = await axios.get(`http://localhost:3000/api/v1/post/getposts?limit=3`);
        const data = await res.data;
        if (res.status==200) {
